...
* Convert x_3_bin into three binary variables without any information loss and put them into a data matrix.
...
The column sum should be the number of infraction, misdimenors and felony
...
```{r}
n = 100
X = matrix(rnorm(n^2), nrow = n, ncol = n) #creating a 100x100 matrix that generates 100^2 random numbers from the normal distribution 
for (i in 1 : n){
  for (j in 1 : n){
    if (runif(1) < 0.3){
      X[i,j] = NA
...
```{r}
apply(X, 2, function(x) sum(na.omit(x!=0)))
```
