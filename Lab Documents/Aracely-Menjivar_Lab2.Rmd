...
flip_matrix= function(X,dim_to_rev=NA){
  ...
  if (is.na(dim_to_rev)){
    if(nr== nc){
      stop("unspecified dim_to_rev and equal rows and cols")
    }
  }
  if(nr > nc){
    dim_to_rev= "r"
...
paste(sample(paste0(strsplit(lorem,split = "\\.\\s" [[1]], ". ")), collaspse= "")
...
my_list$m= list()
my_list$f= list()
my_list$m$mill= strsplit("Zachary, Dylan, Christian, Wesley, Seth, Austin, Gabriel, Evan, Casey, Luis", split = ", ")[[1]]
my_list$m$genX= strsplit(, split = ", ")[[1]]
my_list$m$boomer= strsplit(, split = ", ")[[1]]
my_list$f$mill= strsplit(, split = ", ")[[1]]
my_list$f$genX= strsplit(, split = ", ")[[1]]
my_list$f$boomer= strsplit(, split = ", ")[[1]]
...
X = data.frame(
  treatment = rep(c(letters[1:14]), each=6*3*2*3),
